...
class LEDModel(LEDPreTrainedModel):
    ...
    def forward(
        ...
        return LEDSeq2SeqModelOutput(
            last_hidden_state=decoder_outputs.last_hidden_state, # 1.2
...
class BiasedModelMixin:
    ...
    def set_biases(self, batch_size, seq_len, attribute, init_noise_rate=0.5):
        ...
        self.biases = nn.ParameterList([nn.Parameter(init_noise_rate * torch.randn(batch_size, 1024)) for i in range(seq_len+5)]).cuda().half()

        self.trainable_weights = nn.ParameterList([nn.Parameter(torch.ones(1)) for i in range(seq_len+5)]).cuda().half()
    ...
    def soft_forward(
        ...
        **kwargs
        ...
        if not inference:
            ...
            output_ids, onehot_generates, last_score, soft_generates, logits, gpt_logit = self.soft_greedy_search_with_biases(inputs_embeds, input_ids, pad_token_id=self.config.eos_token_id, inference=True, return_last_score=True, biases=self.biases, use_hidden_states_biases=True, return_logit=True, trainable_weights=self.trainable_weights, **kwargs)
